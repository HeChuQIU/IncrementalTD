...
- **[RESOLVED]**: 具体的"高对比度配色方案"（暗灰背景、彩色点缀）的十六进制颜色代码规范是什么？
  - **方案**: 采用高对比度暗色方案。背景为近纯黑深灰（`#1A1A1A`），玩家方（塔）使用紫色（`#9B59B6`），敌方使用浅灰白（`#DDDDDD`），护甲为纯灰色系（`#333333` 到 `#888888`）。实体绘制采用"暗色填充 + 彩色边框"线框策略，UI 文本使用纯白（`#FFFFFF`），高亮信息使用金色（`#FFD700`）。
